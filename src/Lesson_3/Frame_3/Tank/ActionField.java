...
//    }
    ...
    private boolean processInterceptionField(int y, int x) throws Exception {
        if (y >= 0 && y < 9 && x >= 0 && x < 9) {
            if (battelField.scanQuadrant(y, x).equals("B")) {
                battelField.updateQuadrant(y, x, "");
                return true;
            } else if (battelField.scanQuadrant(y, x).equals("R")) {
                bullet.Destroy();
                return false;
            } else if (battelField.scanQuadrant(y, x).equals("W")) {
                return false;
            } else if (battelField.scanQuadrant(y, x).equals("E")) {
                bullet.Destroy();
...
//                tankRecovery();
    ...
    private boolean processInterception() throws Exception {
        ...
        if (processInterceptionField(y, x) == true) {
            return true;
...
//        repaint();
    ...
    public ActionField() throws Exception {
        ...
        defender = new T34(this, battelField, 64, 512, Direction.TOP);
        ...
        for (int j = 0; j < battelField.getDimentionY(); j++) {
            for (int k = 0; k < battelField.getDimentionX(); k++) {
                if (battelField.scanQuadrant(j, k).equals("B")) {
                    new Brick(Utilites.getCoordinatesFromBattelFieldOfX(j, k), Utilites.getCoordinatesFromBattelFieldOfY(j, k), g);
                } else if (battelField.scanQuadrant(j, k).equals("R")) {
                    new Rock(Utilites.getCoordinatesFromBattelFieldOfX(j, k), Utilites.getCoordinatesFromBattelFieldOfY(j, k), g);
                } else if (battelField.scanQuadrant(j, k).equals("W")) {
                    new Water(Utilites.getCoordinatesFromBattelFieldOfX(j, k), Utilites.getCoordinatesFromBattelFieldOfY(j, k), g);
                } else if (battelField.scanQuadrant(j, k).equals("E")) {
                    new Eagle(Utilites.getCoordinatesFromBattelFieldOfX(j, k), Utilites.getCoordinatesFromBattelFieldOfY(j, k), g);
                }
            }
        }
