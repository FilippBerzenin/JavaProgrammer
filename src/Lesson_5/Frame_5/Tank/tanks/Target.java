...
public class Target {
    ...
    Target (BattleField bf, int x, int y, Direction direction) {
        this.bf = bf;
        this.x = x;
        this.y = y;
...
//        int[] targetLocation = locationOfEagle();
...
//    }
    ...
    private int[] locationOfTarget () {
        int[] targetLocation = new int[2];
        int v = 0;
        int h = 0;
        BFObject bfObject = bf.scanQuadrant(8, 8);
//        System.out.println(bfObject);
        for (int y = 0; y < bf.getBfHeight() / 64; y++) {
            for (int x = 0; x < bf.getBfWidth() / 64; x++) {
                if (bf.scanQuadrant(y, x) instanceof T34  && (v>=0 && v<9 && h>=0 && h<9)) {
                        v = y;
                    targetLocation[0] = y;
                    h = x;
                    targetLocation[1] = x;
                    System.out.println(Arrays.toString(targetLocation));
...
//    int[] targetLocation = locationOfTarget();
//    int eagleX = targetLocation[1];
//    int eagleY = targetLocation[0];
//    int wayX = eagleX;
//    int wayY = eagleY;
